...
class Channel_Encoder(nn.Module):
    ...
    def __init__(self, batchNorm=False, input_size=[240, 416], frame_num=7,
        ...
        for i in range(self.frame_num):
            preconv = nn.ModuleList([conv(self.batchNorm, 6, 64,  kernel_size=3,  stride=1),
...
class Dual_Channel_Attention_Net_Init(nn.Module):

    def __init__(self, batchNorm=False, input_size=[240, 416], mode="train",
                 frame_num=7, lstm_layer=1):
        super(Dual_Channel_Attention_Net_Init, self).__init__()
        self.batchNorm = batchNorm
        self.mode = mode
        self.frame_num = frame_num
        self.lstm_layer = lstm_layer
    ...
    def forward(self, channel_a, channel_b):
        codes_a = self.encoder(torch.concat([channel_a, channel_b], dim=2))

        output_a = self.decoder(codes_a)
        return output_a[:,:3], output_a[:,3:]
...
if __name__ == "__main__":
    ...
    net = Dual_Channel_Attention_Net_Init(input_size=[120, 240], mode='debug')
