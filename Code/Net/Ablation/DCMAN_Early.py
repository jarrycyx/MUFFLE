...
class Channel_Encoder(nn.Module):
    ...
    def __init__(self, batchNorm=False, input_size=[240, 416], frame_num=7,
        ...
        self.LSTM_out = conv(self.batchNorm, 128, 32,
...
class Channel_Decoder(nn.Module):
    ...
    def __init__(self, batchNorm=False, input_size=[240, 416], mode="train", frame_num=7):
        ...
        self.output3 = conv_no_lrelu(
            self.batchNorm, 64, 6, kernel_size=3, stride=1)
...
class Dual_Channel_Attention_Net_Early(nn.Module):
    ...
    def __init__(self, batchNorm=False, input_size=[240, 416], mode="train",
        ...
        self.decoder_a = Channel_Decoder(batchNorm=batchNorm, input_size=input_size,
    ...
    def forward(self, channel_a, channel_b):
        ...
        fused = torch.cat([codes_a, codes_b], dim=1)
        
        out_conv1 = self.conv1_1(self.conv1(fused))
        out_conv2 = self.conv2_1(self.conv2(out_conv1))
        out_conv3 = self.conv3_1(self.conv3(out_conv2))
        out_conv4 = self.conv4_1(self.conv4(out_conv3))

        if self.mode == "debug":
            print("l0 size: ", fused.shape)
            print("l1 size: ", out_conv1.shape)
            print("l2 size: ", out_conv2.shape)
            print("l3 size: ", out_conv3.shape)
            print("l4 size: ", out_conv4.shape)

        output = self.decoder_a([fused, out_conv1, out_conv2, out_conv3, out_conv4])
...
if __name__ == "__main__":
    ...
    net = Dual_Channel_Attention_Net_Early(input_size=[120, 240], mode='debug')
    a, b = net(test_a, test_b)
