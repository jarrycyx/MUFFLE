...
class FastDVDnet_Fn5(nn.Module):
	""" Definition of the FastDVDnet model.
	Inputs of forward():
		xn: input frames of dim [N, C, H, W], (C=3 RGB)
		noise_map: array with noise map of dim [N, 1, H, W]
	...
	def __init__(self):
		super(FastDVDnet_Fn5, self).__init__()
	...
	def forward(self, x):
		...
		# First stage
		x20 = self.temp1(x[:,0], x[:,1], x[:,2])
		x21 = self.temp1(x[:,1], x[:,2], x[:,3])
		x22 = self.temp1(x[:,2], x[:,3], x[:,4])
...
class FastDVDnet_Fn7(nn.Module):
	""" Definition of the FastDVDnet model.
	Inputs of forward():
		xn: input frames of dim [N, C, H, W], (C=3 RGB)
		noise_map: array with noise map of dim [N, 1, H, W]
	...
	def __init__(self):
		super(FastDVDnet_Fn7, self).__init__()
		# Define models of each denoising stage
		self.temp1 = DenBlock(num_input_frames=3)
		self.temp2 = DenBlock(num_input_frames=3)
		# Init weights
