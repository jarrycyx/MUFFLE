...
class Channel_Merge_Decoder_Concat(nn.Module):
    ...
    def forward(self, main_channel, guided_channel):
        ...
        concat4 = torch.cat((main_channel[4], guided_channel[4]), dim=1)
        out_deconv4 = crop_like(self.deconv4(concat4), main_channel[3])
        
        if self.mode=="debug":
            print("main_channel[3] size: ", main_channel[3].shape)

        concat3 = torch.cat((main_channel[3], guided_channel[3], out_deconv4), dim=1)
